...
with
    climate_scrobbles as (
        select
            ...
            row_number() over (
                partition by _scrobbleid
                order by climate.measurement_timestamp desc
            ) as _rn
        ...
        left join
            ...
            and climate.measurement_timestamp > {{ dateadd("hour", -12, "scrobbles.playback_timestamp",) }} 
            and climate.station_name = '{{ var("weather_station_name") }}'
    )
select fk_artist, fk_album, fk_track, fk_date, playback_timestamp, air_temperature, _rn, measurement_timestamp
